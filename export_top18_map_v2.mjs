...
await page.evaluate(() => {
  ...
  block.querySelectorAll('span.P-400-B').forEach(label => {
    // strip the trailing text node, keep the child swatch <span>
...
await el.screenshot({ path: `${OUTPUT_DIR}/Top 18 countries using Forma - no text.png`, omitBackground: true });
console.log('Saved: Top 18 countries using Forma - no text.png');
